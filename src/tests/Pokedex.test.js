...
describe('Testing Component Pokedex', () => {
  ...
  botão "Próximo pokémon" é clicado`, () => {
    ...
    const cardTitle1 = screen.getByText(/charmander/i);
    expect(cardTitle1).toBeDefined();
    expect(cardTitle1).toBeInTheDocument();
    userEvent.click(next);
    const cardTitle2 = screen.getByText(/rapidash/i);
    expect(cardTitle2).toBeInTheDocument();
    userEvent.click(next);
  });
  it('Teste se é mostrado apenas um pokémon por vez.', () => {
    renderWithRouter(<App />);
    const pokemon = screen.getAllByTestId('pokemon-name'); // <p data-testid="pokemon-name>"
